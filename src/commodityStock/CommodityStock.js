...
export function CommodityStock(props) {
    ...
    function changeOrder(column) {
        if (column == orderBy[0]) setOrderBy([column, orderBy[1] ? !1 : 1 ]) // second element is basically an XOR
    ...
    return (
        <>
            ...
            <p><strong>Hint:</strong> Click on a column name to change the order with respect to the selected property.</p>
            <Table>
                <TableHead>
                    <TableRowHead>
                        <TableCellHead>
                        ...
                        <TableCellHead>
                            <p className="cellHeadText" onClick={() => changeOrder(1)}>
                                <Tooltip content="Amount of commodities dispensed so far this period (month)" openDelay={500}>
                                    Consumption {Arrow(1)}
                                </Tooltip>
                            </p>
                        </TableCellHead>
                        <TableCellHead>
                            <p className="cellHeadText" onClick={() => changeOrder(2)}>
                                <Tooltip content="Stock balance of the commodity" openDelay={500}>
                                    End Balance {Arrow(2)}
                                </Tooltip>
                            </p>
                        </TableCellHead>
                        <TableCellHead>
                            <p className="cellHeadText" onClick={() => changeOrder(3)}>
                               <Tooltip content="Amount of commodities to be ordered for the next period (month)" openDelay={500}>
